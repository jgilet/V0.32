...
!     dTx,dsx,.... at faces...                                          
            ...
            i= NI 
            do it=1,ntr 
               dTrx(it,i)= Tr(it,1,j,k,m) - Tr(it,i,j,k,m) 
            end do 
            dsx(i)= s(1,j,k,m) - s(i,j,k,m) 
            dTx(i)= T(1,j,k,m) - T(i,j,k,m) 
            dux(i)= u(1,j,k,m) - u(i,j,k,m) 
            dvx(i)= v(1,j,k,m) - v(i,j,k,m) 
            dwx(i)= w(1,j,k,m) - w(i,j,k,m) 
!     specially for periodic-ew boundaries dsx,dTx... from 0:NI+1       
            i= NI+1 
            do it=1,ntr 
               dTrx(it,i)= Tr(it,2,j,k,m) - Tr(it,1,j,k,m) 
            end do 
            dsx(i)= s(2,j,k,m) - s(1,j,k,m) 
            dTx(i)= T(2,j,k,m) - T(1,j,k,m) 
            dux(i)= u(2,j,k,m) - u(1,j,k,m) 
            dvx(i)= v(2,j,k,m) - v(1,j,k,m) 
            dwx(i)= w(2,j,k,m) - w(1,j,k,m) 
