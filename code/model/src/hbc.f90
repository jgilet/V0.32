...
!     chim1*p(i-1) +chip1*p(i+1) +chjm1p(j-1) ... -ch0*p(i,j,k)= f      
      integer i,j,k,l,im1 
      REAL(kind=rc_kind) :: dtimel,edt,chf(9,NI,NJ),                          &
...
!     dnk= dfloat(NK)                                                   
      const= kaphinv -1.d0 
!     write(6,*) 'const=',const,'kappah=',kappah                        
!     write(6,*) 'in hbc,edt,eg,gprinv,dtinv,dnk',                      
!     &     edt,eg,gprinv,dtinv,dnk                                     
!                                                                       
!                                                                       
...
!     sumvf= sumvf +kappah*cyf(i,j,k) +kaph1*vfbcn(i,k)                 
   50             continue 
...
!     sumvf= sumvf +kappah*cyf(i,0,k) +kaph1*vfbcs(i,k)                 
   60             continue 
                  fn(i,j)= fn(i,j) -eg*sumvf*kaphinv 
               else 
                  sumsjf= 0.d0 
