MODULE particles
  USE header, ONLY : NI,NJ,NK,uf,vf,wf,Jifc,Jjfc,J2d,ux,vy,NPR,wz,PI,dtf,vor,shear,rho,strain,zf,s,parti_file_num,DL,rc_kind
  ...
  REAL(kind=rc_kind) ::  pcx,pcy,pcz,pcr ! the initial position of the particles
...
CONTAINS
  ...
  SUBROUTINE open_parti_files()
    ...
    DO fi = 1, parti_file_num
       ...
       file_id(fi) = 2000 + fi
       OPEN(file_id(fi), file = 'op.parti-'//file_id_char//'.bin', form = 'unformatted', access = 'stream', status = 'replace')
       PRINT*, "open file "//'op.parti-'//file_id_char//'.bin'
    ENDDO
  ...
  SUBROUTINE ini_particles(time)
    ...
    REAL(kind=rc_kind) ::  rand,r,theta
    ...
    CASE (1)
       DO i = 1, NPR/2
          ...
          parti(i)%i = REAL(NI/2) + 0.3*REAL(NI)*r*SIN(theta)
          parti(i)%j = REAL(NJ/2) + 0.3*REAL(NJ)*r*COS(theta)
          parti(i)%k = 22d0
       ENDDO
       DO i = NPR/2, NPR
          CALL RANDOM_NUMBER(r)
    ...
    CASE (3)
       !with streaks in Badin et al. (2011)
       !level k = 5,7,9,11
       !j = (NJ+1)/2 , remember the particle coordinates are on the face, but the rho is on the center
       ! remember set NPR to 12000, 1000x4 (4 level of streaks) + 1000x4 (4levels) + 4000 (3 dimensional random)
       DO k = 5, 11, 2
          DO i = 1, 1000
             CALL RANDOM_NUMBER(r)
             parti(i+(k-5)/2*1000)%i = REAL(NI)*r
             CALL RANDOM_NUMBER(r)
             parti(i+(k-5)/2*1000)%j = REAL((NJ+1)/2)-0.5d0 + r - 0.5d0
             CALL RANDOM_NUMBER(r)
             parti(i+(k-5)/2*1000)%k = REAL(k) + r - 0.5d0
          ENDDO
          !          parti(1+(k-5)/2*1000:1000+(k-5)/2*1000)%k = REAL(k)
       ENDDO
       DO k = 5, 11, 2
          DO i = 1, 1000
             CALL RANDOM_NUMBER(r)
             parti(i+4000+(k-5)/2*1000)%i = REAL(NI)*r
             CALL RANDOM_NUMBER(r)
             parti(i+4000+(k-5)/2*1000)%j = REAL(NJ)*r
             CALL RANDOM_NUMBER(r)
          ENDDO
          parti(4001+(k-5)/2*1000:5000+(k-5)/2*1000)%k = REAL(k) 
       ENDDO
       DO i = 8001, 12000
          CALL RANDOM_NUMBER(r)
          parti(i)%i = REAL(NI)*r
    ...
    CASE (4)
       ...
       !j = (NJ+1)/2 , remember the particle coordinates are on the face, but the rho is on the center
       ! remember set NPR to 12000, 1000x4 (4 level of streaks) + 1000x4 (4levels) + 4000 (3 dimensional random)
       PRINT*, "# ini particle positions, use the case 4"
       DO k = 1, 4
          DO i = 1, 3000
             CALL RANDOM_NUMBER(r)
             parti(i+(k-1)*3000)%i = DBLE(k*NI/5) + r
             CALL RANDOM_NUMBER(r)
             parti(i+(k-1)*3000)%j = DBLE(NJ/2) +  r
             CALL RANDOM_NUMBER(r)
             parti(i+(k-1)*3000)%k = 11d0 + r
          ENDDO
       ENDDO
    CASE (5)
       !with streaks in Badin et al. (2011)
       ...
       DO k = 12, 36, 8
          DO i = 1, 1000
             CALL RANDOM_NUMBER(r)
             parti(i+(k-12)/8*1000)%i = DBLE(NI)*r
             CALL RANDOM_NUMBER(r)
             parti(i+(k-12)/8*1000)%j = DBLE((NJ+1)/2)
             CALL RANDOM_NUMBER(r)
             parti(i+(k-12)/8*1000)%k = DBLE(k)
          ENDDO
          !   parti(1+(k-5)/2*1000:1000+(k-5)/2*1000)%k = REAL(k)
       ENDDO
       DO k = 12, 36, 8
          DO i = 1, 1000
             CALL RANDOM_NUMBER(r)
             parti(i+4000+(k-12)/8*1000)%i = REAL(NI)*r
             CALL RANDOM_NUMBER(r)
             parti(i+4000+(k-12)/8*1000)%j = REAL(NJ)*r
             CALL RANDOM_NUMBER(r)
          ENDDO
          parti(4001+(k-12)/8*1000:5000+(k-12)/8*1000)%k = REAL(k) 
       ENDDO
       DO i = 8001, 12000
          CALL RANDOM_NUMBER(r)
          parti(i)%i = REAL(NI)*r
       ...
       ENDDO
    CASE (6)
       !24 streaks on 4 levels k =17 27 37 47
       PRINT*, '# initilize particles to 4 levels k =17 27 37 47. Each level has 6 streaks with 500 particles.'
       itmp=-1
       npr_eachline = 500
       DO k = 17, 47, 10
          DO j = -8,12,4 
             ...
             DO i = 1,npr_eachline
                CALL RANDOM_NUMBER(r)
                parti(itmp*npr_eachline+i)%i = DBLE(NI)*r
             ENDDO
             parti(itmp*npr_eachline+1:itmp*npr_eachline+npr_eachline)%j = DBLE(NJ/2)+DBLE(j)
          ENDDO
          parti(itmp*npr_eachline+1:itmp*npr_eachline+npr_eachline)%k = DBLE(k)
       ENDDO
     case (7)
       !24 streaks on 4 levels ,for NK=32
       PRINT*, '# initilize particles to 4 levels k =10 14 18 22. Each level has 6 streaks with 500 particles.'
       itmp=-1
       npr_eachline = 200
       DO k = 10, 22, 4
          DO j = -4,6,2
  ...
  SUBROUTINE get_parti_vel(time)
    ...
    DO ip = 1, NPR
       ...
       IF (parti(ip)%j < NJ .AND. parti(ip)%j > 0 .AND. &
          ...
          !dzf = (parti(ip)%z - zf(ifc,jfc,kfc)) / ( zf(ifc,jfc,kfc+1) - zf(ifc,jfc,kfc) )

          dic = parti(ip)%i - ic + 0.5d0
          djc = parti(ip)%j - jc + 0.5d0
          dkc = parti(ip)%k - kc + 0.5d0
          !calcuate the zonal velocity 
          CALL sigma2z(parti(ip)%i,parti(ip)%j+0.5d0,parti(ip)%k+0.5,swap1)
          ...
          dz = (swap1 - swap2) / ( swap3 - swap2 )
          parti(ip)%z=swap1*DL
          CALL interp_trilinear(dic,djc,dz,wf_ex(ic:ic+1,jc:jc+1,kfc:kfc+1),parti(ip)%w)
          !CALL interp_trilinear(dic,djc,dkf,wf_ex(ic:ic+1,jc:jc+1,kfc:kfc+1),parti(ip)%w)

          !diagnose other properties
          CALL sigma2z(parti(ip)%i+0.5d0,parti(ip)%j+0.5d0,parti(ip)%k+0.5d0,swap1)
          CALL sigma2z(parti(ip)%i+0.5d0,parti(ip)%j+0.5d0,dble(kc),swap2)
          CALL sigma2z(parti(ip)%i+0.5d0,parti(ip)%j+0.5d0,dble(kc+1),swap3)
          dz = (swap1 - swap2) / ( swap3 - swap2 )

          CALL interp_trilinear(dic,djc,dz,vor(ic:ic+1,jc:jc+1,kc:kc+1),parti(ip)%vor)
          CALL interp_trilinear(dic,djc,dz,rho(ic:ic+1,jc:jc+1,kc:kc+1),parti(ip)%rho)
          CALL interp_trilinear(dic,djc,dz,shear(ic:ic+1,jc:jc+1,kc:kc+1),parti(ip)%shear)
          CALL interp_trilinear(dic,djc,dz,strain(ic:ic+1,jc:jc+1,kc:kc+1),parti(ip)%strain)
!!$          if (ip == 8001) then
!!$          print*, 'dkc,dz=',dkc,dz
...
!!$          endif
          ...
          !CALL interp_trilinear(dic,djc,dkc,s(ic:ic+1,jc:jc+1,kc:kc+1,0),parti(ip)%vor)
  ...
  SUBROUTINE parti_forward()
    ...
    DO i = 1, NPR
       ...
       IF (parti(i)%j<0d0 .OR. parti(i)%j>NJ .OR. parti(i)%k>NK .OR. parti(i)%k<0d0 ) THEN
          PRINT*, "particles coordinates are wrong, iPR=",i,"j,k",parti(i)%j,parti(i)%k
          !stop
       ENDIF
