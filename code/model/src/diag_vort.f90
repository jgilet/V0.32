...
!                                                                       
     ...
     &              (v(i,j,k+1,0) -v(i,j,k-1,0))*wy(i,j,k) )            
               strain(i,j,k)= sqrt((udx - vdy)**2 +(vdx+ udy)**2)      &
     &              *UL*LENinv                                          
                                                                        
!=               udz = 0.5*( (u(i,j,k+1,0) -u(i,j,k-1,0))*wz(i,j,k) )   
!=               wdx = 0.5*((w(i+1,j,k,0)-w(i-1,j,k,0))*ux(i,j) +       
!=     &              (w(i,j+1,k,0) -w(i,j-1,k,0))*vx(i,j) +            
!=     &              (w(i,j,k+1,0) -w(i,j,k-1,0))*wx(i,j,k) )          
!=               z2(i,j,k)= udz*UL*DLinv -wdx*WL*LENinv                 
!=                                                                      
!=               wdy= 0.5*((w(i+1,j,k,0)-w(i-1,j,k,0))*uy(i,j) +        
!=     &              (w(i,j+1,k,0) -w(i,j-1,k,0))*vy(i,j) +            
!=     &              (w(i,j,k+1,0) -w(i,j,k-1,0))*wy(i,j,k) )          
!=               vdz= 0.5*( (v(i,j,k+1,0) -v(i,j,k-1,0))*wz(i,j,k) )    
!=               z1(i,j,k)= wdy*WL*LENinv -vdz*UL*DLinv                 
            end do 
         end do 
      end do 
!     vertical SHEAR                                                    
      do j=1,NJ 
         do i=1,NI 
            k=1 
            vz= (v(i,j,k+1,0)-v(i,j,k,0))*wz(i,j,k) 
            uz= (u(i,j,k+1,0)-u(i,j,k,0))*wz(i,j,k) 
            ...
            k=NK 
            vz= (v(i,j,k,0)-v(i,j,k-1,0))*wz(i,j,k) 
            uz= (u(i,j,k,0)-u(i,j,k-1,0))*wz(i,j,k) 
            shear(i,j,k)= sqrt(udz*udz + vdz*vdz)*UL/DL 
         end do 
      end do 
