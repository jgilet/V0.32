subroutine viscous(udif,vdif,wdif,sdif,Tdif,Trdif,n) 
  !     ---------------------------------------------                     
  USE header
  !     use level n                                                       
  ...
  REAL(kind=rc_kind) ::                                                  &
       dudxfc(0:NI),dvdxfc(0:NI),dwdxfc(0:NI),dsdxfc(0:NI),         &
       dTdxfc(0:NI),dTrdxfc(ntr,0:NI),                              &
       dudyfc(0:NJ),dvdyfc(0:NJ),dwdyfc(0:NJ),dsdyfc(0:NJ),         &
       dTdyfc(0:NJ),dTrdyfc(ntr,0:NJ),vyj,uxi,fac,ubylsq,wbylsq 
  REAL(kind=rc_kind) :: Kx,Ky(NJ),e1inv,yfrac,y0,y0inv,yend,yy,          &
       KxPassTr                                                     
...
!==         Ky(j)= 20.0                                                 
  ...
  do k=1,NK 
     do i=1,NI 
        ...
        do j=1,NJ 
           udif(i,j,k)= Ky(j)*vy(i,j)*(dudyfc(j)-dudyfc(j-1)) 
           vdif(i,j,k)= Ky(j)*vy(i,j)*(dvdyfc(j)-dvdyfc(j-1)) 
           wdif(i,j,k)= Ky(j)*vy(i,j)*(dwdyfc(j)-dwdyfc(j-1)) 
           sdif(i,j,k)= Ky(j)*vy(i,j)*(dsdyfc(j)-dsdyfc(j-1)) 
           Tdif(i,j,k)= Ky(j)*vy(i,j)*(dTdyfc(j)-dTdyfc(j-1)) 
           do it=1,ntr 
              Trdif(it,i,j,k)= KxPassTr*vy(i,j)*                     &
                   (dTrdyfc(it,j)-dTrdyfc(it,j-1))   
           end do
        end do
     end do
  end do
...
!     periodic-ew boundaries                                            
        ...
        do i=1,NI 
           udif(i,j,k)= udif(i,j,k)                                 &
                + Kx*ux(i,j)*(dudxfc(i)-dudxfc(i-1))                
           vdif(i,j,k)= vdif(i,j,k)                                 &
                + Kx*ux(i,j)*(dvdxfc(i)-dvdxfc(i-1))                
           wdif(i,j,k)= wdif(i,j,k)                                 &
                + Kx*ux(i,j)*(dwdxfc(i)-dwdxfc(i-1))                
           sdif(i,j,k)= sdif(i,j,k)                                 &
                + Kx*ux(i,j)*(dsdxfc(i)-dsdxfc(i-1))                
           Tdif(i,j,k)= Tdif(i,j,k)                                 &
                + Kx*ux(i,j)*(dTdxfc(i)-dTdxfc(i-1))                
           do it=1,ntr 
              Trdif(it,i,j,k)= Trdif(it,i,j,k)                      &
                   + KxPassTr*ux(i,j)*(dTrdxfc(it,i)-dTrdxfc(it,i-1))
           end do
        end do
     end do
  end do
