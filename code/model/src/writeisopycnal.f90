subroutine writeisopycnal(frame_int,sig,stressx,T,rho,vor,strain, &
  ...
  !     ------------------------------------------------------------------
USE header,ONLY : NI,NJ,NK,ntr,nconsume,rc_kind
  !     writes the property T on isopycnal sig                            
#include "netcdf.inc"                                                                        
      integer i,j,k,step,frame_int,it,n 
      REAL(kind=rc_kind) :: DL,zmax,LEN,zeuphotic,dtfsec,seval 
                                                                        
      REAL(kind=rc_kind) :: dens(NK),depth(NK),tracer(NK),stressx(NJ),       &
     ...
     &     freqN2(0:NI+1,0:NJ+1,0:NK+1),sig(ntr),sigup,sigdn,dsig       
      REAL(kind=rc_kind) :: zf(0:NI+1,0:NJ+1,-1:NK+1),xc(0:NI+1),yc(0:NJ+1) 
      REAL(kind=rc_kind) :: bs1(NK),cs1(NK),ds1(NK) 
