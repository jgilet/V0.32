...
!     tightness=4, gives the needed b_xx to make b_xx H/ (2f^2) < 1.    
      ...
      integer  i,j,k,it,iseed,npm1,iter,iter2 ,np
      parameter (np=33)
      double precision rhovert(np),Tvert(np),svert(np),dep(np),         &
           depoff(np),drho(np) 
...
!      tightness= 0.3d0   !used in inith_fixdh                          
      ...
      call spline (np,depoff,svert,bs1,cs1,ds1) 
      call spline (np,depoff,tvert,bT1,cT1,dT1) 
            ...
            do k=0,NK+1 
               z= DL*zc(i,j,k) 
                  if (z.ge.-mldepth) then 
                     Tbkgrnd =                                          &
     &               seval(np,-1.*mldepth,depoff,tvert,bT1,cT1,dT1)     &
     &                       - (z+mldepth)*dtdz
                     sbkgrnd =                                          &
     &               seval(np,-1.*mldepth,depoff,svert,bs1,cs1,ds1)     &
     &                       - (z+mldepth)*dsdz
!     &               seval(np,-1.*mldepth,depoff,svert,bs1,cs1,ds1)     &
!     &                       - (z+mldepth)*bfsqbkgrnd*R0/(gpr*10.)      
                  else 
                     Tbkgrnd =                                          &
     &                    seval(np,z,depoff,tvert,bT1,cT1,dT1)        
                     sbkgrnd =                                          &
     &                    seval(np,z,depoff,svert,bs1,cs1,ds1)        
!     &                    seval(np,z,depoff,svert,bs1,cs1,ds1)        
                  end if 
!==                                                                     
                  T(i,j,k,0)=  Tbkgrnd 
