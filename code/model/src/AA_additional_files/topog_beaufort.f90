subroutine topog 
  ...
  !     fits a spline to the described topog and computed Ddx(i)          
      ...
      real*8 ::dynew,dep,seval,LbyD,DLinv 
      real*8 ::ynew(-nfacm1:NJnew+nfac),                        &
...
!     continental shelf                                                 
         ...
         elseif (ynew(j).lt.120.) then 
            dep= 60. + 0.75*(ynew(j)-100.)**2 -(0.1/4.)*(ynew(j)-120.)**2 
         elseif (ynew(j).lt.140.) then 
            dep= 360. +30*(ynew(j)-120.) 
         elseif (ynew(j).lt.160.) then 
            dep= 1280. -0.8*(160.-ynew(j))**2 
         else 
            dep= 1280.d0 
         end if 
!         write(6,*) ynew(j),dep                                             
