subroutine diffusion(sdif,Tdif,udif,vdif,m,step) 
  !     ---------------------------------------------                     
  USE header
...
!     Use the same value for mixing tracers and momentum                
     ...
     &     vdif(NI,NJ,NK),Kdudzb,Kdvdzb,Kdudzt,Kdvdzt,wzkth,fact,Cp     
      real*8 RR,fac,facb,dfactor,Kzmax,facy,rhoinv,diff,      &
...
!     mix buoyancy too                                                  
                                                            !Tanh profil
      parameter (RR= 0.000d0, Kzmax= 1.d-3, KzmaxTr=1.d-3) 
...
!==                                                                     
      ...
      do j=1,NJ 
         if (dabs(stressx(j)-stressmax).lt.0.001) stressx(j)=stressmax 
         if (dabs(stressx(j)).lt.0.001) stressx(j)=0.d0 
      end do 
...
! jbw 09/20/2011
! linear damping along south and north walls.
do j = 0, NJ+1
   r = 1d0/(0.864d0*1d0)*tanh((2d0*dble(j)-dble(NJ+1))/dble(NJ+1))**16 !damping scale = 2 days, 1 day is 0.864
   !sdif(:,j,:) = sdif(:,j,:) - LEN*r*Jac(:,j,:)*u(:,:,:,m)
   udif(:,j,:) = udif(:,j,:) - LEN*r*Jac(1:NI,j,1:NK)*u(1:NI,j,1:NK,m)
   vdif(:,j,:) = vdif(:,j,:) - LEN*r*Jac(1:NI,j,1:NK)*v(1:NI,j,1:NK,m)
enddo
...
!     Assumes that rho is evaluated just prior to this call             
      ...
      integer n1 
      parameter (n1=3) 
      real*8 fac,bvfreq,grho,RiCr,Ri,vshear,DLinv 
      real*8 dudz(0:NK),dvdz(0:NK),drdz(0:NK) 
